...
const getAssetFolder = (assetType, resources) => {
    switch (assetType) {
        case "native":
...
const copyFiles = async (assetType, files, sourceFolder, resources) => {
...
const copyAssets = async (assetType, resources) => {
    await Promise.all(getAssetKeys(assetType).map(async (assetKey) => {
...
const unpackNativeModules = async (resources) => {
    if (!sea.isSea())
        return;
    await copyAssets("native", resources);
};
const permissionNativeModules = async (resources) => {
    if (!sea.isSea())
        return;
...
const unpackSpecs = async (resources) => {
    if (!sea.isSea()) {
...
const unpackShellFiles = async (resources) => {
    if (!sea.isSea()) {
...
const setUnpackedVersion = async (resources) => {
    const version = getVersion();
    await fsAsync.writeFile(resources.version, version, "utf-8");
};
export const checkUnpackedVersion = async () => {
...
export const unpackResources = async (resourcesPath = allResourcesPath) => {
