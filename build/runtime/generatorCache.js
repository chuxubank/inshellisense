...
const getStateKey = (generator, tokens, activeToken, cwd) => {
...
const getState = (key, explicitCache) => {
    ...
    const state = { explicitCache, generationVersion: 0 };
    generatorStates.set(key, state);
    if (generatorStates.size > maxGeneratorStates) {
        const oldestKey = generatorStates.keys().next().value;
        if (oldestKey != null)
            generatorStates.delete(oldestKey);
    }
    return state;
};
const triggerMatches = (trigger, newToken, oldToken) => {
    if (trigger == null)
        return false;
    if (typeof trigger === "function")
        return trigger(newToken, oldToken);
    const matchChanged = (matches) => {
...
const waitForDebounce = async (signal) => {
...
const executeGeneratorSafely = async (generator, tokens, cwd, signal) => {
    try {
...
const generate = async (state, generator, tokens, activeToken, cwd, debounce, signal) => {
    const generationKey = JSON.stringify([tokens, activeToken, cwd]);
    if (state.inFlight?.key === generationKey && state.inFlight.signal?.aborted !== true)
    ...
    try {
        ...
        if (state.inFlight?.promise === generation)
            state.inFlight = undefined;
    }
};
const refreshInBackground = async (state, generator, tokens, activeToken, cwd) => {
    try {
        const result = await generate(state, generator, tokens, activeToken, cwd, false);
        if (!result.cacheable || !result.current)
            return;
...
export const getGeneratorSuggestions = async (generator, tokens, activeToken, cwd, debounce, signal) => {
    signal?.throwIfAborted();
...
export const clearGeneratorState = () => {
    generatorStates.clear();
};
export const clearImplicitGeneratorState = () => {
    for (const [key, state] of generatorStates) {
        if (!state.explicitCache)
            generatorStates.delete(key);
    }
};
