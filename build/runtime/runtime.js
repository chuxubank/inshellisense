...
import figSpecList, { diffVersionedCompletions as figVersionedSpeclist,
...
const speclist = figSpecList.filter((spec) => !ignoredSpecs.some((name) => spec === name || spec.startsWith(name + "/")));
const versionedSpeclist = figVersionedSpeclist.filter((spec) => !ignoredSpecs.some((name) => spec.startsWith(name)));
...
loadSpecsSet(speclist, versionedSpeclist, specResourcesPath);
const loadedSpecs = {};
const loadSpec = async (cmd, signal) => {
...
const lazyLoadSpec = async (key, signal) => {
    signal?.throwIfAborted();
...
export const loadLocalSpecsSet = async () => {
    ...
    await Promise.allSettled(specsPath.map((specPath) => {
        ...
        return import(importPath)
            ...
            .catch((e) => {
            log.debug({ msg: `no local specs imported from '${specPath}', this will not break the current session`, e: e.message, specPath });
        });
    }));
};
export const getSuggestions = async (cmd, cwd, shell, signal) => {
    ...
    try {
        ...
        if (result == null)
            return;
        if (result.suggestions.length == 0 && !result.argumentDescription)
            return;
...
export const clearTransientSuggestionState = () => clearImplicitGeneratorState();
const prefixMatches = (values, prefix) => {
    let low = 0;
...
const genSubcommand = async (command, parentCommand, signal) => {
    ...
    switch (typeof subcommand.loadSpec) {
        case "function": {
            ...
            if (partSpec instanceof Array) {
                const locationSpecs = (await Promise.all(partSpec.map((s) => lazyLoadSpecLocation(s, signal)))).filter((s) => s != null);
                ...
                const locationSingleSpec = await lazyLoadSpecLocation(partSpec, signal);
                parentCommand.subcommands[subcommandIdx] = {
                    ...subcommand,
        ...
        case "string": {
            const spec = await lazyLoadSpec(subcommand.loadSpec, signal);
            parentCommand.subcommands[subcommandIdx] = {
                ...subcommand,
                ...(getSubcommand(spec) ?? []),
                loadSpec: undefined,
...
const runOption = async (tokens, allTokens, option, subcommand, cwd, shell, persistentOptions, acceptedTokens, signal) => {
    if (tokens.length === 0) {
    ...
    return runSubcommand(tokens.slice(1), allTokens, subcommand, cwd, shell, persistentOptions, acceptedTokens.concat({
        ...activeToken,
        isPersistent,
    }), undefined, undefined, signal);
};
const runArg = async (tokens, allTokens, args, subcommand, cwd, shell, persistentOptions, acceptedTokens, fromOption, fromVariadic, signal) => {
    signal?.throwIfAborted();
    if (args.length === 0) {
    ...
    if (args.every((a) => a.isOptional)) {
        ...
        if (nextSubcommand != null) {
            return runSubcommand(tokens.slice(1), allTokens, nextSubcommand, cwd, shell, persistentOptions, getPersistentTokens(acceptedTokens.concat(activeToken)), undefined, undefined, signal);
        }
    }
    ...
    return runArg(tokens.slice(1), allTokens, args.slice(1), subcommand, cwd, shell, persistentOptions, acceptedTokens.concat(activeToken), fromOption, false, signal);
};
const runSubcommand = async (tokens, allTokens, subcommand, cwd, shell, persistentOptions = [], acceptedTokens = [], argsDepleted = false, argsUsed = false, signal) => {
    signal?.throwIfAborted();
    if (tokens.length === 0) {
    ...
    const nextSubcommand = await genSubcommand(activeToken.token, subcommand, signal);
    if (nextSubcommand != null) {
        return runSubcommand(tokens.slice(1), allTokens, nextSubcommand, cwd, shell, getPersistentOptions(persistentOptions, subcommand.options), getPersistentTokens(acceptedTokens.concat(activeToken)), undefined, undefined, signal);
    }
